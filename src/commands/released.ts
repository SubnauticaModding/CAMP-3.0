...
export default class implements Command {
  ...
  async execute(message: Discord.Message, args: string[]) {
    ...
    if (parseInt(args[0]).toString() != args[0]) return this.error(message, "Invalid arguments. Expected a valid mod idea ID as the first argument.");

    const modidea = mod_ideas.getModIdea(parseInt(args[0]));

    if (!modidea) return this.error(message, "Invalid arguments. Expected a valid mod idea ID as the first argument.");

    const matches = [...(args[1] ?? "").matchAll(/https?:\/\/nexusmods.com\/(subnautica(?:belowzero)?)\/mods\/(\d+)/g)];

    if (matches.length != 1) return this.error(message, "Invalid arguments. Expected a valid NexusMods link as the second argument.");

    const game = matches[0][1];
    const id = matches[0][2];

    if ((game != "subnautica" && game != "subnauticabelowzero") || parseInt(id).toString() != id)
      return this.error(message, "Invalid arguments. Expected a valid NexusMods link as the second argument.");

    const modinfo = await nexus.getModInfo(parseInt(id), game);

    if (!modinfo.available || modinfo.status != "published") return this.error(message, `Invalid mod. Mod \`${game}/${id}\` is not available.`);
    ...
    const replymsg = await message.channel.send(new Discord.MessageEmbed({
      description: `Mod idea \`#${ideaID}\` has been marked as released.\nClick [here](${(await newIdeaMsg).url}) to view it.`,
      color: "GREEN"
    }));
  ...
  error(message: Discord.Message, text: string) {
