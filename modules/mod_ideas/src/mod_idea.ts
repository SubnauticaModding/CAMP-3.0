...
import("../../../extensions/String/replaceAsync");
...
export default class ModIdea {
  ...
  public async sendOrEdit(channel: string) {
    ...
    if (oldMessage && oldMessage?.channel.id == channel) {
      ...
      if (oldMessage && !ignored_messages.includes(oldMessage.id)) {
  ...
  public static async updateReportMessage() {
    const channel = await bot.channels.fetch(config.modules.mod_ideas.channels.report.channel) as Discord.TextChannel;
    try {
      var message = await channel.messages.fetch(config.modules.mod_ideas.channels.report.message);
