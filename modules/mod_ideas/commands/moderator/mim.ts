...
commands.push(new Command({
  ...
  description: `Adds or removes the <@&${config.permissions[CommandPermission.ModIdeasManager]}> role to/from a person.`,
  ...
  execute: async (message: Discord.Message, args: string[]) => {
    ...
    if (author.roles.cache.keyArray().filter(r => r == config.permissions[CommandPermission.ModIdeasManager]).length == 0) {
      try {
        ...
        embeds.success(message, `You have given <@${author.id}> the <@&${config.permissions[CommandPermission.ModIdeasManager]}> role.`);
      } catch (e) {
        console.error(e);
      ...
      try {
        ...
        embeds.success(message, `You have removed the <@&${config.permissions[CommandPermission.ModIdeasManager]}> role from <@${author.id}>.`);
      } catch (e) {
        console.error(e);
        embeds.error(message, "An error has occurred. The bot probably doesn't have permission to edit the roles of that user or access the wanted role.");
      }
